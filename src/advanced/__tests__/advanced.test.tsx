...
describe("advanced > ", () => {
  ...
  describe("자유롭게 작성해보세요.", () => {
    test("새로운 유틸 함수를 만든 후에 테스트 코드를 작성해서 실행해보세요", () => {
      ...
      const result = calculateTotalPrice(1000, 5, 0.1);
      expect(result).toBe(4500);
    });

    test("useDiscount 훅 테스트", () => {
      ...
      const { result } = renderHook(() =>
        useProductEdit({ onProductUpdate: mockOnProductUpdate })
      );
      ...
      // 상품명 수정 테스트
      act(() => {
        result.current.handleProductNameUpdate("1", "수정된 상품명");
      });

      expect(result.current.editingProduct?.name).toBe("수정된 상품명");
