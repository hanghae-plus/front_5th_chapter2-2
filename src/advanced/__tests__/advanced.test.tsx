...
import {
  ...
  formatPrice,
  truncate,
} from "../../refactoring/utils/stringUtils";
...
describe("advanced > ", () => {
  ...
  describe("유틸 함수 테스트", () => {
    ...
    describe("formatDiscountType", () => {
      test("할인 타입이 amount이면 '금액'을 반환해야 한다", () => {
        expect(formatDiscountType("amount")).toBe("금액");
      });

      test("할인 타입이 percentage이면 '비율'을 반환해야 한다", () => {
        expect(formatDiscountType("percentage")).toBe("비율");
      });
    });

    describe("formatDiscountRate", () => {
      test("소수 할인율을 백분율 문자열로 변환해야 한다", () => {
        expect(formatDiscountRate(0.1)).toBe("10%");
        expect(formatDiscountRate(0.25)).toBe("25%");
        expect(formatDiscountRate(1)).toBe("100%");
      });
    });
  });

  describe("useProducts hook 테스트", () => {
    const initialProducts: Product[] = [
      { id: "1", name: "사과", price: 1000, stock: 10, discounts: [] },
      { id: "2", name: "바나나먹고싶다", price: 500, stock: 20, discounts: [] },
    ];
    ...
    test("상품 추가", () => {
      ...
      expect(result.current.products[2].name).toBe("당군");
    });
  });
});
