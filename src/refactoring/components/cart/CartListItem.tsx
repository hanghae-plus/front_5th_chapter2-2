...
function CartListItem({ cart, onChangeQuantity, onClickRemove }: Props) {
	...
	return (
		<div key={cart.product.id} className="flex justify-between items-center bg-white p-3 rounded shadow">
      <div>
      ...
      <div>
        <button className="bg-gray-300 text-gray-800 px-2 py-1 rounded mr-1 hover:bg-gray-400" onClick={() => onChangeQuantity(cart.product.id, cart.quantity - 1)}>
          -
        </button>
        <button className="bg-gray-300 text-gray-800 px-2 py-1 rounded mr-1 hover:bg-gray-400" onClick={() => onChangeQuantity(cart.product.id, cart.quantity + 1)}>
          +
