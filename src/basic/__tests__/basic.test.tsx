...
import { AppProvider } from '../../refactoring/app/AppProvider';
...
import { CouponProvider, ProductProvider, useCart, useCoupons, useProducts } from '../../refactoring/hooks';
...
const TestAdminPage = () => {
  return (
    <AppProvider products={mockProducts} coupons={mockCoupons}>
      <AdminPage />
    </AppProvider>
  );
};
...
describe('basic > ', () => {
  ...
  describe('useProducts > ', () => {
    ...
    test('특정 제품으로 초기화할 수 있다.', () => {
      const wrapper = ({ children }: { children: React.ReactNode }) => (
        <ProductProvider value={initialProducts}>{children}</ProductProvider>
      );
  ...
  describe('useCoupons > ', () => {
    test('쿠폰을 초기화할 수 있다.', () => {
      const wrapper = ({ children }: { children: React.ReactNode }) => (
        <CouponProvider value={mockCoupons}>{children}</CouponProvider>
      );
